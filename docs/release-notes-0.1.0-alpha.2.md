...
Public Alpha 2 adds the complete English interface requested for community
testing. It remains an unsigned **Public Alpha**, and the product is still
**PARTIAL**.

## Changes since Public Alpha 1

- Complete English translation for the workspace, diagnostics, installation,
  logs, TF2 setup, project creation, confirmations, status messages, tooltips,
  accessibility labels and native file dialogs.
- Automatic German or English selection from the operating-system language.
- Visible `DE` / `EN` switch in the top bar.
- Explicit language choices are saved locally and restored on the next launch.
- Real paths, file names, diagnostic codes and TF2 log content remain unchanged
  so technical evidence is never altered by translation.
